# recordR, t1
    ...
    Code
      recodR(df, namelist = list(diag = list(KOL = "DX", Astma = c("DC123", "DC2"),
      AMI = c("DG123", "DG234"))), match = "start")
    Output
...
# recordR, t2
    ...
    Code
      df %>% factR(c(diag, type, type2)) %>% recodR(list(diag = list(KOL = "DX123",
        Astma = c("DC123", "DC234"), AMI = list("DG123", "DG234"))), match = "exact")
    Condition
      Warning:
...
# recordR, t3
    ...
    Code
      df %>% recodR(list(split = list(one = "1", four = "4", ten = "10", fifteen = "15")),
      match = "boundary")
    Output
          diag  type type2   split
